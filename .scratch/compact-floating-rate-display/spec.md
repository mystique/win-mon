...
实现约 3:1 的深色胶囊，主体尺寸为 **132×48 DIP**。左侧圆环内显示 Download Rate 数值和单位；右侧上方显示 Upload Rate，下方显示近期 Download Rate 曲线。
...
- 主体 132×48 DIP，圆角半径 24 DIP，约 3:1。左侧环外径约 39 DIP，主体内留约 4 DIP 边距，线宽约 1.64 DIP，使用圆头端点。
...
- 文字尺寸起点：下载数值约 16.4 DIP、单位约 9.8 DIP、上传约 13.1 DIP。最终以真实尺寸可读性为准，而非机械复制放大图。
...
- 比例为 0.5 时，上传终点在顶部，下载终点在底部；比例为 1 时，两者终点均在左端。左右分界处必须留约 2 DIP 可见间隙，暗轨道与亮弧均断开、端点为圆头，不能改变半环归属或增长方向。
...
- 设计收敛记录：保留第二款下载优先布局，采用第三款约 3:1 比例，恢复项目深色系，尺寸由 132×48 DIP 缩至 132×44 DIP，圆环改为上传在上、下载在下且均从右端开始。
...
主体从 132×44 DIP 适度放大为 132×48 DIP，文字及图形同比放大约 9%，阴影边距仍为 3 DIP。对照原型，左右两端的上下半环及暗轨道均保留约 2 DIP 间隙。以上覆盖早期尺寸收敛记录。
...
双向趋势线加粗至约 1.75 DIP（原约 1.09 DIP），使用单调三次 Hermite 插值连接现有历史采样点。经过每个采样点，保留峰谷位置及幅度，不额外平滑数据；单调段不产生越界过冲。新增孤立尖峰保留与插值范围位图回归，Release 构建及 3/3 CTest 通过。

### 2026-09-20 默认下载、悬停双行

默认以原字号垂直居中显示蓝色下载速率，上传隐藏。悬停时下载平滑下移，青绿上传渐显；移出反向恢复。沿用 180ms／220ms 可反向过渡，下载始终不透明且字号不变。回归检查默认上传隐藏、下载持续更新及中间布局帧，3/3 CTest 通过。原运行程序占用 build/Release/WinMon.exe，本次 Release 改为构建至 build/hover-preview/Release/WinMon.exe。
