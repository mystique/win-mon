...
An optional, borderless, always-on-top 132x48 DIP dark capsule. The right side stacks a small green Upload Rate above a larger blue Download Rate, matching their ring colors. By default only Download Rate is visible, vertically centered at its normal font size. Pointer hover moves it down while Upload Rate fades in above it; pointer exit restores the centered download-only view. The reversible transition takes 180ms in and 220ms out; curves remain visible. Disabled Windows animations switch layouts immediately.  Green upload and blue download trends overlap behind both readings across the full right-hand height, sharing a recent peak scale. The left ring has independent green upper and blue lower activity arcs with small gaps at both horizontal joints. A soft center-out pulse inside the ring reflects relative network activity; it stays clear of the ring strokes. While visible and active, an approximately 30 FPS native timer advances the pulse without adding Rate Samples; idle and hidden displays stop it, and Windows' disabled client-area animation setting uses a static activity glow. Direct2D/DirectWrite render at the display monitor's DPI with per-pixel alpha. The shadow occupies at most 3 DIP per side and passes mouse input through. The Rate Font family and styles are shared, but sizes fit this compact layout without changing the saved font or the Rate Strip. It has no taskbar or Alt-Tab entry, drags without activation, and opens the Operator Menu on right-click. It remains available while the Rate Strip is hidden. Graphical history resets when Network to Monitor changes or falls back. Visibility and absolute desktop position persist; first use and an unusable restored position place it at the primary work area's upper-right corner.
