...
Superseded in part by ADR-0007: operator toggles (Launch at Login, Rate Strip Right-Click Menu) now persist under `HKCU\Software\Win Mon` and the Run key. The rest of this decision stands — NIC Selection is not persisted and every launch starts from defaults.
