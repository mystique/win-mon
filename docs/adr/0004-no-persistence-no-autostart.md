...
Superseded in part by ADR-0007: operator toggles (Launch at Login, Right-Click Speed Text for This Menu — originally shipped as "Rate Strip Right-Click Menu") now persist under `HKCU\Software\Win Mon` and the Run key. The rest of this decision stands — the network choice, since relabelled Network to Monitor, is not persisted and every launch starts from defaults.
